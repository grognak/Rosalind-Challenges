...
"AUGCUUCAGAAAGGUCUUACG" are 2, 5, 6, 15, 17, and 18). The symbol at position i of s is
denoted by s[i].

A substring of s can be represented as s[j:k], where j and k represent the starting and
ending positions of the substring in s; for example, if s = "AUGCUUCAGAAAGGUCUUACG", then
s[2:5] = "UGCU".
...
if __name__ == "__main__":
    print("ROSALIND Finding a Motif in DNA (SUBS)")
    with open("../inputs/rosalind_subs.txt", "r") as f:
        s = f.readline().strip()
        t = f.readline().strip()
        print(subs(s, t))